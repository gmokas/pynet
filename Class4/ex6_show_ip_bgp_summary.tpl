...
Start
  ^BGP.*identifier ${BGP_ROUTER_ID}, local.*number ${LOCAL_AS}
  ^Neighbor.*State/PfxRcd -> output

output
  ^${NEIGHBOR}\s+\d\s+${REMOTE_AS}\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+${UPDOWN}\s+${STATE_PREFIX}.*$$ -> Record
